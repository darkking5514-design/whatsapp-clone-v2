...
export default function GroupChat() {
  ...
  const handleReply = (msg) => { setReplyTo(msg); setShowMessageMenu(null); };
  const handleForward = (msg) => { setForwardMessage(msg); setShowMessageMenu(null); setShowForwardModal(true); };
  const handleDelete = (msg, deleteFor) => {
    // Similar to private chat – we can implement later
    ...
    <div className="flex h-dvh bg-[#111b21] overflow-hidden">
      ...
      <div className="flex flex-col h-full w-full bg-whatsapp-chatbg relative min-h-0">
              ...
              <div key={m._id} className={`flex ${isSent ? 'justify-end' : 'justify-start'} group`}>
                <div className="relative max-w-[75%]">
                  ...
                  <div className={`rounded-lg px-3 py-2 text-sm shadow ${
                    ...
                    {/* Render media similar to ChatWindow */}
                    {m.messageType === 'image' && m.mediaUrl && (
                      <img src={`${SOCKET_URL}${m.mediaUrl}`} className="rounded-md max-h-64 object-cover" />
                    )}
                    {m.messageType === 'video' && m.mediaUrl && (
                      <video src={`${SOCKET_URL}${m.mediaUrl}`} controls className="rounded-md max-h-64" />
                    )}
                    {m.messageType === 'audio' && m.mediaUrl && (
                      <div className="flex items-center gap-3 min-w-[160px]">
                        <button onClick={() => toggleAudioPlay(m._id, `${SOCKET_URL}${m.mediaUrl}`)}>
                          {audioPlaying === m._id ? <Pause size={18} /> : <Play size={18} />}
                        </button>
                        <div className="flex-1 h-1 bg-gray-500 rounded-full">
                          <div className="h-1 bg-whatsapp-green rounded-full" style={{ width: '0%' }} id={`progress-${m._id}`} />
                        </div>
                        <span className="text-xs text-gray-300">{formatDuration(m.duration)}</span>
                        <audio
                  ...
                  {/* Message menu similar to ChatWindow */}
                  <button
                    onClick={() => setShowMessageMenu(showMessageMenu === m._id ? null : m._id)}
                    className="absolute -top-2 -right-2 p-1 bg-[#202c33] rounded-full opacity-0 group-hover:opacity-100"
                  >
                  ...
                  {showMessageMenu === m._id && (
                    <div className="absolute -top-8 right-6 bg-[#202c33] rounded-lg shadow-lg p-1 z-10 flex gap-1 border border-[#3b4a54]">
                      <button onClick={() => handleReply(m)} className="p-1.5 hover:bg-[#2a3942] rounded"><Reply size={14} /></button>
                      <button onClick={() => handleForward(m)} className="p-1.5 hover:bg-[#2a3942] rounded"><Forward size={14} /></button>
                      {isSent && (
                        <>
                          <button onClick={() => handleDelete(m, 'me')} className="p-1.5 hover:bg-[#2a3942] rounded"><Trash2 size={14} /></button>
                          <button onClick={() => handleDelete(m, 'everyone')} className="p-1.5 hover:bg-red-500/20 rounded"><Trash2 size={14} className="text-red-400" /></button>
                        </>
                      )}
                      {!isSent && (
                        <button onClick={() => handleDelete(m, 'me')} className="p-1.5 hover:bg-[#2a3942] rounded"><Trash2 size={14} /></button>
        ...
        <div className="bg-[#202c33] px-3 py-2 flex items-center gap-2 flex-shrink-0">
          ...
          />
          <button onClick={() => fileInputRef.current?.click()} className="text-gray-300 p-2 hover:text-white" disabled={uploading}>
            <Paperclip size={20} />
          </button>

          {!isRecording && !audioURL && (
            <button onClick={startRecording} className="text-gray-300 p-2 hover:text-white">
              <Mic size={20} />
            </button>
          )}
          ...
          {isRecording && (
            <div className="flex items-center gap-2 bg-[#2a3942] rounded-full px-3 py-1 flex-1">
              ...
              <button onClick={cancelRecording} className="text-red-400 hover:text-red-300 p-1"><X size={18} /></button>
              <button onClick={stopRecording} className="bg-whatsapp-green text-black rounded-full p-1"><Square size={16} fill="black" /></button>
            </div>
          )}
          ...
          {audioURL && !isRecording && (
            <div className="flex items-center gap-2 bg-[#2a3942] rounded-full px-3 py-1 flex-1">
              ...
              <button onClick={cancelRecording} className="text-red-400 hover:text-red-300 p-1"><X size={18} /></button>
              <button onClick={sendVoiceMessage} className="bg-whatsapp-green text-black rounded-full p-1"><Send size={16} /></button>
            </div>
          )}
          ...
          {!isRecording && !audioURL && text.trim() && (
            <button onClick={handleSend} className="bg-whatsapp-green text-black rounded-full p-2 hover:opacity-90">
              <Send size={18} />
            </button>
          )}
        </div>
      ...
      {showInfo && group && (
        <GroupInfoModal
          ...
          onUpdate={() => {
            // refresh group and messages
            api.get(`/groups/${groupId}`).then(res => setGroup(res.data));
          }}
        />
      )}

      {/* Forward Modal (simplified) - same as ChatWindow */}
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
          <div className="bg-[#202c33] rounded-lg p-4 max-w-md w-full max-h-[80vh]">
            <div className="flex justify-between items-center mb-3">
              ...
              <button onClick={() => { setShowForwardModal(false); setForwardMessage(null); }} className="text-gray-400 hover:text-white"><X size={20} /></button>
            </div>
